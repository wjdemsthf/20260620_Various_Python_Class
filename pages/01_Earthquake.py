...
학생들이 관심 있는 지역의 지진 발생 횟수와 강도를 직접 분석하고, 대피 훈련에 과학적 흥미를 더할 수 있도록 돕습니다.
""")
...
if df.empty:
    ...
    # (4) 교육용 지진 예측 및 확률 분석 모듈
    st.markdown("---")
    st.subheader("🔮 교육용 지진 확률 예측 (수학적 리스크 분석)")
    
    with st.expander("💡 학생들을 위한 필수 과학 지식 읽어보기", expanded=True):
        st.info("""
        **"지진은 정확히 언제, 어디서 일어날지 초 단위로 맞출 수 있을까요?"**  
        현재 현대 과학 기술로는 특정 지진의 정확한 날짜와 시각을 예측하는 것이 불가능합니다.  
